...
infix fun Faktum<LocalDate>.etter(tidligsteDato: Faktum<LocalDate>) =
    EnkelSubsumsjon(
        object : Regel {
...
infix fun Faktum<LocalDate>.ikkeEtter(tom: Faktum<LocalDate>) =
    EnkelSubsumsjon(
        object : Regel {
...
infix fun Faktum<LocalDate>.før(senesteDato: Faktum<LocalDate>) =
    EnkelSubsumsjon(
        object : Regel {
...
infix fun Faktum<LocalDate>.ikkeFør(senesteDato: Faktum<LocalDate>) =
    EnkelSubsumsjon(
        object : Regel {
...
infix fun Faktum<LocalDate>.førEllerLik(senesteDato: Faktum<LocalDate>) =
    EnkelSubsumsjon(
        object : Regel {
...
infix fun Faktum<LocalDate>.etterEllerLik(senesteDato: Faktum<LocalDate>) =
    EnkelSubsumsjon(
        object : Regel {
...
class PeriodeSubsumsjonBuilder internal constructor(
    ...
    infix fun og(tom: Faktum<LocalDate>) =
        "periode innenfor sjekk".alle(
            faktum ikkeFør fom,
            faktum ikkeEtter tom,
        )
}

infix fun Faktum<Inntekt>.minst(terskel: Faktum<Inntekt>) =
    EnkelSubsumsjon(
        object : Regel {
...
infix fun <T : Comparable<T>> Faktum<T>.er(other: T) =
    EnkelSubsumsjon(
        Er(other),
        this,
    )

infix fun <T : Valg> Faktum<T>.inneholder(valg: T) =
    EnkelSubsumsjon(
        Inneholder(valg),
        this,
    )

private class Inneholder<T : Valg>(
    private val other: T,
) : Regel {
...
private class Er<T : Comparable<T>>(
    private val other: T,
) : Regel {
...
infix fun <T : Comparable<T>> Faktum<T>.erIkke(other: T) =
    EnkelSubsumsjon(
        ErIkke(other),
        this,
    )

private class ErIkke<T : Comparable<T>>(
    private val other: T,
) : Regel {
...
infix fun GeneratorFaktum.med(deltre: DeltreSubsumsjon) =
    DeltreSubsumsjon(
        "Alle ${this.navn} med",
        GeneratorSubsumsjon(
            ErIkke(0),
...
infix fun GeneratorFaktum.har(deltre: DeltreSubsumsjon) =
    DeltreSubsumsjon(
...
private class Utfylt : Regel {
    ...
    override fun resultat(fakta: List<Faktum<*>>) = fakta[0].erBesvart()
...
private class Minst(
    ...
    override fun resultat(fakta: List<Faktum<*>>) = (fakta[0] as Faktum<Int>).svar() >= tall
