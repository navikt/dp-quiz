...
class UtledetFaktum<R : Comparable<R>> internal constructor(
    ...
    override fun besvar(
        verdi: R,
        identer: String?,
    ): Faktum<R> = throw IllegalArgumentException("Kan ikke besvare sammensatte faktum")
    ...
    override fun rehydrer(
        r: R,
        besvarer: String?,
    ): Faktum<R> = throw IllegalArgumentException("Kan ikke rehydrere sammensatte faktum")
    ...
    internal fun addAll(fakta: List<Faktum<*>>) =
        fakta.forEach { underordnet ->
    ...
    override fun accept(visitor: FaktumVisitor) {
        ...
        if (erBesvart()) {
            ...
            visitor.preVisit(this, id, avhengigeFakta, avhengerAvFakta, underordnede, type(), regel)
        }
    ...
    internal fun erDefinert(
        faktaList: List<Faktum<*>>,
        indeks: Int,
    ) = underordnede.all { faktum ->
