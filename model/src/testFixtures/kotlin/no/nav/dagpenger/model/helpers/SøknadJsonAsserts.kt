...
@JsonAssertMarker
class MedSøknad private constructor(
    private val søknad: JsonNode,
...
class MedFakta(
    ...
    fun harAntallBesvarte(value: Int) = assertEquals(value, fakta.count { it.has("svar") }, "Feil antall faktum i seksjon")

    fun dokument(
        navn: String,
        block: MedFaktum.() -> Unit,
    ) = faktum(navn, block).also {
        it.erType("dokument")
    }
...
open class MedFaktum(
    ...
    fun erReadOnly(readOnly: Boolean = true) = assertEquals(readOnly, faktum["readOnly"].asBoolean(), "Faktum $navn var ikke readOnly")

    fun harRoller(vararg rolle: String) = harRoller(rolle.toList())

    private fun harRoller(roller: List<String>) =
        faktum.get("roller").toSet().map { it.asText() }.also {
    ...
    open fun erBesvartMed(tekst: String) = assertEquals(tekst, faktum["svar"].asText(), "Faktum $navn er besvart med")
...
class MedGeneratorFaktum(
    ...
    fun svar(
        i: Int,
        block: MedFakta.() -> Unit,
    ) = MedFakta(svar[i - 1]).also { block(it) }
...
open class MedLandFaktum(
    ...
    fun grupper(
        sjekkAlle: Boolean,
        block: MedGruppeLandFaktum.() -> Unit,
    ) {
    ...
    inner class MedGruppeLandFaktum(
        ...
        private val grupper =
            faktum["grupper"].associate { fakta ->
